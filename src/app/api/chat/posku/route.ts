...
const RAG_DOCS_URL =
  process.env.POSKU_RAG_DOCS_URL || 'https://posku-rag-docs.kubido.workers.dev';
...
export async function POST(req: Request) {
  ...
  let text = '';
  try {
    const res = await fetch(`${RAG_DOCS_URL}/api/chat`, {
      method: 'POST',
    ...
    const lastUser = uiToSimple(messages)
      .reverse()
      .find((m) => m.role === 'user');
    text = `Maaf, layanan tanya informasi sedang tidak dapat diakses. Pertanyaan Anda: "${lastUser?.content ?? ''}"`;
  }
